...
public class MainActivity extends Activity {
	...
	private final static String PROFILE_CREATED = "ProfileCreated";

	/**
	 * Substitute you own sender ID here. This is the project number you got
	 * from the API Console, as described in "Getting Started."
	 */
	String SENDER_ID = "69583278348";
		...
		if (isFirstTime()) {
			...
			setRoute.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					editor.putString("Name", nameText.getEditableText()
							.toString());
					editor.putString("EmailID", emailText.getEditableText()
							.toString());
					editor.putString("Number", numberText.getEditableText()
							.toString());
	...
	public void startApp() {
		...
		try {
			...
			if (gps.canGetLocation()) {
				...
				userInformation.setCord(userInformation.new Coordinates(gps
						.getLatitude(), gps.getLongitude()));
				// pollQRT.sendFirstCurrentLocation(getApplicationContext(),
				// getIntent(), userInformation, gps);
		...
		// R.layout.list_item));
		autoCompView2.setAdapter(new PlacesAutoCompleteAdapter(this,
				R.layout.list_item));
		/*
		 * autoCompView1.setOnItemClickListener(new OnItemClickListener() {
		 * 
		 * @Override public void onItemClick(AdapterView<?> parent, View view,
		 * int position, long id) { // TODO Auto-generated method stub Button
		 * setRoute = (Button) findViewById(R.id.button1);
		 ...
		 * .resultList.get(position)).get(); } catch (InterruptedException e) {
		...
		setRoute.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				try {
					...
					CameraPosition cameraPosition = new CameraPosition.Builder()
							.target(new LatLng(destination.latitude,
									destination.longitude)).zoom(12).build();
					googleMap.animateCamera(CameraUpdateFactory
							.newCameraPosition(cameraPosition));
					Toast.makeText(MainActivity.this,
							"Route is send to Server and has been set",
							Toast.LENGTH_SHORT).show();
					Button setRoute = (Button) findViewById(R.id.button1);
					...
					userInformation.setCord(userInformation.new Coordinates(gps
							.getLatitude(), gps.getLongitude()));
					// pollQRT.sendCurrentLocation(getApplicationContext(),
					// getIntent(), userInformation, gps);
				} catch (Exception e) {
					Toast.makeText(MainActivity.this, e.getMessage(),
							Toast.LENGTH_SHORT).show();
		...
		sos.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				userInformation.setCord(userInformation.new Coordinates(gps
						.getLatitude(), gps.getLongitude()));
				PollQRT pollQRT = new PollQRT();
				pollQRT.sendSOS(getApplicationContext(), getIntent(),
						userInformation);
				Toast.makeText(getApplicationContext(),
						"SOS sent to server. We are sending help",
	...
	private boolean isFirstTime() {
		...
		boolean isProfieCreated = preferences
				.getBoolean(PROFILE_CREATED, false);
		if (!isProfieCreated) {
			// first time
